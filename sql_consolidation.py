...
# 1. Inspection des colonnes disponibles dans le fichier de features
cols_features = [col[0] for col in con.execute(f"DESCRIBE SELECT * FROM '{file_features}'").fetchall()]
print(f"Colonnes détectées dans dataset_final_features_advanced : {cols_features[:5]}... (+{len(cols_features)-5} autres)")
